...
The extraction pipeline processes lawsuit documents to extract structured events with temporal information, party mentions, and provenance tracking.

#### Prerequisites

1. **vLLM Server** - Start the NuExtract3 model server:
```bash
...
2. **Parsed Documents** - Documents must be parsed with Docling first to generate `.docling.json` files in the case directory.

#### Running Extraction

```bash
# Extract all stages (0-4) for a case
uv run scripts/extract.py --case-id <case-id> --stages 0-4

# Extract specific stages
uv run scripts/extract.py --case-id <case-id> --stages 2

# Force re-run of existing stages
uv run scripts/extract.py --case-id <case-id> --stages 0-4 --force

# Use custom config
uv run scripts/extract.py --case-id <case-id> --stages 0-4 --config custom.toml
```

#### Pipeline Stages

The extraction pipeline consists of 5 stages:
...
| **4** | Proto-events (predicate + typed edges) | GLiNER-Relex (optional) |

**Stage Outputs** (in `data/cases/<case-id>/stages/`):
- `00_segments.json` - Document segmentation with paragraph labels
- `01_metadata.json` - Document metadata and party seeds from captions
- `02_registry.json` - Canonical party registry with mentions and aliases
- `03_spans.json` - All entity spans extracted by GLiNER
- `04_protoevents.json` - Priority segments and proto-events (if enabled)

#### Configuration

Edit `config/extraction.toml` to tune the pipeline:

**GLiNER Labels** (highly tunable):
```toml
[gliner]
labels = [
  "temporal expression",
...
```toml
[registry]
fuzzy_match_threshold = 88  # 0-100, higher = stricter matching
```
...
temperature = 0.0  # Must be 0 for deterministic extraction
...
[maverick]
model = "sapienzanlp/maverick-mes-ontonotes"
enabled = true

[relex]
enabled = false  # GLiNER-Relex not yet available
```
...
from lawsuit_parser.extraction.schemas import SegmentsArtifact, RegistryArtifact
from pathlib import Path

# Initialize store
store = ArtifactStore("case-id", Path("data/cases"))
...
# Read canonical text
text = store.read_canonical_text("doc_000")

# Access data
print(f"Found {len(segments.segments)} segments")
print(f"Found {len(registry.parties)} parties")
print(f"Found {len(registry.mentions)} party mentions")

# Every span has exact char offsets
for mention in registry.mentions[:5]:
    start = mention.span.char_start
    end = mention.span.char_end
    # Verify: text[start:end] == mention.span.text
    assert text[start:end] == mention.span.text
```

#### Global Validation

The pipeline automatically validates that all spans are valid:
```python
# This check runs after extraction
